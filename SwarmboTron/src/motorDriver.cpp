...
void MotorDriver::moveRobotRealistic()
{
    ...
    while(goalAngle <0) goalAngle += PI * 2;
...
//        }
//    }
    while(currentAngle >= 2*PI) currentAngle -= PI * 2;

    double left = 0;
    double right = 0;
    if(onLocation)
    {
        double maxSpeed = map(difference,0,2 * PI,udpData.speed/2,udpData.speed);
        if(difference > 0)
        {
            left = maxSpeed;
            right = -maxSpeed;
    ...
    else
    {
        ...
        double rideAngle = 0.3*PI;
        if(abs(difference) < 0.1 * PI)
        {
            left = maxSpeed;
            right = maxSpeed;
        }
